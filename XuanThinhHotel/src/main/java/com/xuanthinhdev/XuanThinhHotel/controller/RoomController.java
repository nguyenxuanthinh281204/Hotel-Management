package com.xuanthinhdev.XuanThinhHotel.controller;

import com.xuanthinhdev.XuanThinhHotel.dto.Response;
import com.xuanthinhdev.XuanThinhHotel.service.interfac.IBookingService;
import com.xuanthinhdev.XuanThinhHotel.service.interfac.IRoomService;
...
public class RoomController {
    ...
    public ResponseEntity<Response> getAvailableRoomsByDateAndType(
        ...
        if(checkInDate == null || roomType == null || roomType.isBlank() || checkOutDate == null){
            Response response = new Response();
            response.setStatusCode(400);
            response.setMessage("Please provide values for all field(checkInDate, roomType, checkOutDate)");
            return ResponseEntity.status(response.getStatusCode()).body(response);
    ...
    public ResponseEntity<Response> updateRoom(@PathVariable Long roomId,
                                               @PathVariable(value = "photo",required = false) MultipartFile photo,
                                               @RequestParam(value = "roomType", required = false) String roomType,
                                               @RequestParam(value = "roomPrice", required = false) BigDecimal roomPrice,
                                               @RequestParam(value = "roomDescription", required = false) String roomDescription
                                               ){
